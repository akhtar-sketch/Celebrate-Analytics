...
export function MetaIcon({ size = 16, className }: IconProps) {
  return (
    <svg
      width={size}
      height={size}
...
export function TikTokIcon({ size = 16, className }: IconProps) {
  return (
    <svg
      ...
      className={className}
    >
      {/* TikTok note shape — light grey fill, adapts to context */}
      <path
...
export function PlatformIcon({ platform, size = 16 }: { platform: string; size?: number }) {
  ...
  if (platform === 'tiktok') return <TikTokIcon size={size} className="text-ink-2" />
  return null
}

export const PLATFORM_CHART_COLORS: Record<string, string> = {
  google:  '#3b82f6',   // blue-500  — visually distinct
  meta:    '#f59e0b',   // amber-500 — warm contrast
  tiktok:  '#ec4899',   // pink-500  — energetic
}
